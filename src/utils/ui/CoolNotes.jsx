...
export class CoolNotes extends Component {
  ...
  static get_menu_options = (segment_data) => {
    ...
    const fancy_edit =
      segment_data.props && segment_data.props.editor_type === "fancy"
  ...
  go_to_edit = () => {
    if (CoolNotes.edit_owner) {
      ...
      document.removeEventListener(
        "focusout",
        CoolNotes.edit_owner.focusout_handler,
      );
  ...
  render() {
    ...
    const { placeholder, style_extra, editor_type, value, on_update_props } =
      this.props;
    if (!edit_mode) {
      return value ? (
        <RegularText
        ...
        <PromptText onClick={(e) => this.go_to_edit()}>
          click to begin
        </PromptText>
      );
    }
