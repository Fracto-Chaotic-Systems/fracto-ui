...
export class LogViewer extends Component {
  ...
  componentDidUpdate(previous_props, previous_state) {
    const logs_changed =
      previous_state.logs_data.console_lines !==
      this.state.logs_data.console_lines;
    const auto_scroll_enabled =
      this.state.auto_scroll && !previous_state.auto_scroll;
  ...
  disable_auto_scroll = () => {
    if (!this.auto_scrolling && this.state.auto_scroll)
      this.setState({ auto_scroll: false });
  };
  ...
  toggle_timestamps = (event) => {
    ...
    AppSettings.on_settings_changed({
      [this.props.timestamp_key]: show_timestamps,
    });
  ...
  updated_label = () => {
    const records = this.state.logs_data.records || [];
    const last_message = [...records]
      .reverse()
      .find((record) => record.timestamp);
    const updated_at = last_message?.timestamp || this.state.last_refreshed_at;
    const time_ago =
      !updated_at || Date.now() - new Date(updated_at).getTime() < 60000
        ? AppText.get(KEY_LOG_UPDATED_MOMENTS)
        : relative_time(new Date(updated_at).toISOString());
    return (
      <>
        <strong>{AppText.get(KEY_LOG_UPDATED)}:</strong> <em>{time_ago}</em>
      </>
    );
  };
      ...
      <>
        ...
        <styles.CenteredBlock>
          ...
          >
            ...
            >
              <label
                style={{ marginLeft: "0.5rem", textTransform: "uppercase" }}
              >
                <input
                  type="checkbox"
