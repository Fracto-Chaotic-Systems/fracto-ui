...
export class ComplexQuarternary {
  ...
  constructor(p0, p1) {
    ...
    if (Array.isArray(p0)) {
      ...
      for (let l_index = 0; l_index < left_part.length; l_index++) {
        this.left_part[MAX_LEFT_DIGITS - left_part.length + l_index] =
          left_part[l_index];
      }
  ...
  to_string = () => {
    ...
    let digit = 0;
    while (
      this.left_part[digit] === 0 &&
      this.left_part[digit + 1] === 0 &&
      digit < this.left_part.length - 1
    ) {
      digit++;
    }
    if (digit > 0) {
  ...
  square_sum = (p) => {
    ...
    for (let op1_index = 0; op1_index < MAX_RIGHT_DIGITS; op1_index++) {
      ...
      for (let op2_index = 0; op2_index < MAX_RIGHT_DIGITS; op2_index++) {
        this.staging[base_staging - op2_index] += this.quick_mult(
          factor,
          this.right_part[base_index_right - op2_index],
        );
      }
    }
    ...
    for (let op1_index = 0; op1_index < MAX_LEFT_DIGITS; op1_index++) {
      ...
      for (let op2_index = 0; op2_index < MAX_LEFT_DIGITS; op2_index++) {
        this.staging[base_staging - op2_index] += this.quick_mult(
          factor,
          this.left_part[base_index_keft - op2_index],
        );
      }
    }
