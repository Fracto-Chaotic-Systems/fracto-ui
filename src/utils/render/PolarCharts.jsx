...
export class PolarCharts extends Component {
  ...
  componentDidMount() {
    ...
    const width_px =
      Math.round(page_settings[KEY_COMPS_WIDTH_PX] / WIDTH_FACTOR) -
      WIDTH_OFFSET_PX;
    const height_px =
      Math.round(page_settings[KEY_COMPS_HEIGHT_PX] / HEIGHT_FACTOR) -
      HEIGHT_OFFSET_PX;
  ...
  initialize = () => {
    const current_click_point_info = get_click_point_info({});
    this.setState({
  ...
  render_charts = (width_px, height_px) => {
    ...
    const { click_point, orbital_points, Q_core_neg, pattern, in_cardioid } =
      click_point_info;
    let r_data_set = [];
    if (!pattern) {
    ...
    return (
      <styles.GraphWrapper key={"scatter-chart"} style={chart_style}>
        <styles.ZoomerStealth ref={zoomer_stealth_ref} style={stealth_style}>
          {chart}
  ...
  polar_caption_text = () => {
    ...
    const subject = (
      <styles.CaptionSubject key={"caption-subject"}>
        {"AMPLITUDE"}
    ...
    const qualifier = (
      <styles.CaptionQualifier key={"polar-caption-qualifier"}>
        {", in the range of "}
      </styles.CaptionQualifier>
    );
    const min_y_styled = (
      <styles.ClickPoint key={"polar-caption-click-point-min"}>
        {`${min_y}`}
      </styles.ClickPoint>
    );
    const max_y_styled = (
      <styles.ClickPoint key={"polar-caption-click-point-max"}>
        {`${max_y}`}
      </styles.ClickPoint>
    );
    const and = (
      <styles.CaptionQualifier key={"polar-caption-qualifier-and"}>
        {" and "}
      </styles.CaptionQualifier>
    );
    return [subject, qualifier, min_y_styled, and, max_y_styled];
  };
  ...
  render() {
    ...
    const caption = (
      <styles.CaptionWrapper key={"polar-caption"}>
        {caption_text}
      </styles.CaptionWrapper>
    );
    const rendered_area = [caption, chart_stack, wrapped_zoomer];
    return (
      <AppErrorBoundary fallback={rendered_area}>
        {rendered_area}
      </AppErrorBoundary>
    );
  }
