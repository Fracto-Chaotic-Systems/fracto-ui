...
import { click_point_chart, escape_points_chart } from "./PatternsUtils";
...
export class ScatterCharts extends Component {
  ...
  render_scatter_chart = () => {
    ...
    const { click_point, pattern, orbital_points, in_cardioid, Q_core_neg } =
      click_point_info;
    if (pattern) {
  ...
  render_chart = (content, width_px, height_px) => {
    ...
    return (
      <AppErrorBoundary fallback={content}>
        <styles.GraphWrapper style={chart_style}>{content}</styles.GraphWrapper>
      </AppErrorBoundary>
    );
  };
