...
export class FractoOrbitalChart extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (focal_y_changed || focal_x_changed || ready_changed) {
      ...
      const in_animation_changed =
        prevProps.in_animation !== this.props.in_animation;
  ...
  init_animation = () => {
    ...
    if (in_animation && fracto_values) {
      const interval = setInterval(() => {
        let next_index = 1;
        if (
          this.state.animation_index <
          fracto_values.orbital_points.length - 1
        ) {
  ...
  other_sets = () => {
    const { fracto_values, Q_core_neg, Q_core_pos, animation_index } =
      this.state;
    const { in_animation } = this.props;
