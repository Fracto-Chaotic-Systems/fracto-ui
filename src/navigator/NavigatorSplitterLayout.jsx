...
export class NavigatorSplitterLayout extends Component {
  ...
  render() {
    const { main_splitter_pos, legend_splitter_pos, steps_splitter_pos } =
      this.state;
    const { bounding_rect, splitter_keys, frame_settings, frame_settings_key } =
      this.props;
    ...
    const legend_pane_style = {
      top:
        steps_bounding_rect.height + bounding_rect.top + SPLITTER_WIDTH_PX + 2,
      left: bounding_rect.left,
      width: main_splitter_pos - page_splitter_pos,
      height:
        bounding_rect.height - bounding_rect.top - steps_bounding_rect.height,
      background: BACKGROUND_FIELD_GRADIENT,
    };
    const all_panes = [
      <styles.FixedWrapper style={step_pane_style} key={"steps-pane"}>
        <NavigatorSteps
      ...
      <styles.FixedWrapper style={field_pane_style} key={"field-pane"}>
        <NavigatorField
      ...
      <styles.FixedWrapper style={legend_pane_style} key={"legend-pane"}>
        <NavigatorLegend
