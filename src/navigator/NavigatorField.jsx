...
const ZOOM_FACTOR_MINOR = 1.618;
const ZOOM_FACTOR_MAJOR = ZOOM_FACTOR_MINOR * ZOOM_FACTOR_MINOR;
...
export class NavigatorField extends Component {
  ...
  adjust_canvas_size = () => {
    ...
    const largest_width_px = Math.min(
      bounding_rect.width,
  ...
  on_wheel = (e) => {
    ...
    if (disabled) {
      console.log("on_wheel: rendering disabled");
      return;
    }
    ...
    frame_settings.scope =
      e.deltaY > 0 ? scope * zoom_factor : scope / zoom_factor;
    AppSettings.on_settings_changed({
  ...
  on_click = (e) => {
    ...
    const disabled = AppSettings.get(KEY_NAVIGATOR_DISABLED);
    if (disabled) {
      console.log("on_click: rendering disabled");
      return;
    }
    const { focal_point, scope } = frame_settings;
