...
const STRATEGY_OPTIONS = [
  {
    value: "turbo",
    label: KEY_NAVIGATOR_STRATEGY_TURBO,
    help: KEY_NAVIGATOR_STRATEGY_TURBO_HELP,
  },
  {
    value: "legacy",
    label: KEY_NAVIGATOR_STRATEGY_LEGACY,
    help: KEY_NAVIGATOR_STRATEGY_LEGACY_HELP,
  },
];
...
export class NavigatorLegend extends Component {
  ...
  paste_focal_point = () => {
    const { frame_settings_key } = this.props;
    const clipboard_data = AppSettings.get(KEY_CLIPBOARD_DATA);
    if (clipboard_data.frame_settings) {
      AppSettings.on_settings_changed({
        [frame_settings_key]: clipboard_data.frame_settings,
      });
    }
  };
  ...
  render_focal_point = (focal_point) => {
    ...
    let copy_icon = [];
    if (frame_settings.scope) {
      copy_icon = (
        <main_styles.InlineHover
          title={"copy to clipboard"}
          onClick={() => copy_clipboard("frame_settings", frame_settings)}
          style={icon_style}
        >
          {copy}
        </main_styles.InlineHover>
    ...
    const paste_icon = (
      <main_styles.InlineHover
        ...
        onClick={this.paste_focal_point}
        style={icon_style}
      >
        {paste}
      </main_styles.InlineHover>
    );
    return [rendered, <main_styles.HalfRemSpacer />, copy_icon, paste_icon];
  };
  ...
  render_stats = () => {
    ...
    const table_data = [
      ...
      },
      {
        ...
        value: [
          ...
          AppSettings.get(KEY_NAVIGATOR_STRATEGY_SETTING),
        ],
      },
    ];
    const stats_style = {
    ...
    return (
      <styles.StatsWrapper style={stats_style}>
        <CoolTable
  ...
  render_strategy = (strategy) => {
    ...
    return (
      <CoolStyles.InlineBlock style={select_style}>
        <CoolSelect
  ...
  render() {
    ...
    return [
      <CoolStyles.InlineBlock>
        <CoolStyles.Block style={{ textAlign: "center", marginTop: "0.5rem" }}>
          <main_styles.HoverBlueButton onClick={(e) => this.on_zoom_out(e)}>
            zoom out
          </main_styles.HoverBlueButton>
        </CoolStyles.Block>
        <main_styles.QuarterRemDown />
        <CoolStyles.Block>{transitor}</CoolStyles.Block>
        <CoolStyles.Block style={{ textAlign: "center", margin: "0" }}>
          <main_styles.HoverBlueButton onClick={(e) => this.on_zoom_in(e)}>
            zoom in
          </main_styles.HoverBlueButton>
        </CoolStyles.Block>
      </CoolStyles.InlineBlock>,
