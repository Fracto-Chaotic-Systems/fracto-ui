...
export class TilesTest extends Component {
  ...
  componentDidMount() {
    ...
    const tab_index =
      Number.isInteger(saved_tab) && saved_tab >= 0 && saved_tab <= 1
        ? saved_tab
        : 0;
  ...
  render() {
    ...
    const top =
      container_ref.current?.getBoundingClientRect().top || TITLE_BAR_HEIGHT_PX;
    const available_height = Math.max(0, rendered_height - top);
    const tab_content_height = Math.max(
      0,
      available_height - TAB_HEADER_HEIGHT_PX,
    );
    return [
      ...
      >
        <CoolTabs
          labels={[
            AppText.get(KEY_TILES_TEST_BENCHMARKS),
            AppText.get(KEY_TILES_TEST_ANIMATION),
          ]}
          tab_index={tab_index}
