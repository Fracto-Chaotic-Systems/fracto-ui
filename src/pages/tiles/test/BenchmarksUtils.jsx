...
const LEGEND_ENTRIES = [
  {
    key: "legacy_min_ms",
    text_key: KEY_TILES_TEST_LEGACY_MIN,
    color: STRATEGY_COLORS.legacy[0],
  },
  {
    key: "legacy_median_ms",
    text_key: KEY_TILES_TEST_LEGACY_MEDIAN,
    color: STRATEGY_COLORS.legacy[1],
  },
  {
    key: "legacy_max_ms",
    text_key: KEY_TILES_TEST_LEGACY_MAX,
    color: STRATEGY_COLORS.legacy[2],
  },
  {
    key: "turbo_min_ms",
    text_key: KEY_TILES_TEST_TURBO_MIN,
    color: STRATEGY_COLORS.turbo[0],
  },
  {
    key: "turbo_median_ms",
    text_key: KEY_TILES_TEST_TURBO_MEDIAN,
    color: STRATEGY_COLORS.turbo[1],
  },
  {
    key: "turbo_max_ms",
    text_key: KEY_TILES_TEST_TURBO_MAX,
    color: STRATEGY_COLORS.turbo[2],
  },
];
...
const robust_step_value = (values, metric_index) => {
  ...
  const trimmed = sorted.slice(
    trim_count,
    Math.max(trim_count + 1, sorted.length - trim_count),
  );
  ...
  const log_mean = Math.exp(
    winsorized.reduce((sum, value) => sum + Math.log(Math.max(0.1, value)), 0) /
      winsorized.length,
  );
  ...
  for (const strategy of ["legacy", "turbo"]) {
    ...
    for (const [source_key, source_fixtures] of source_entries) {
      source_fixtures.sort(
        (left, right) => (get_step(left) || 0) - (get_step(right) || 0),
      );
      METRICS.forEach((metric, metric_index) => {
        const points = source_fixtures
          .map((fixture) => ({
            x: get_step(fixture),
        ...
        const step_values = [...new Set(points.map((point) => point.x))].sort(
...
export const chart_options = (
        ...
        generateLabels: (chart) =>
          LEGEND_ENTRIES.map((entry) => {
            const dataset_indexes = chart.data.datasets
              .map((dataset, index) =>
                dataset.legend_key === entry.key ? index : -1,
              )
              .filter((index) => index >= 0);
            const hidden =
              dataset_indexes.length > 0 &&
              dataset_indexes.every((index) => !chart.isDatasetVisible(index));
            return {
  ...
  scales: {
    x: {
      type: "linear",
      title: { display: true, text: step_label },
      ticks: { precision: 0 },
    },
    y: {
      type: "logarithmic",
