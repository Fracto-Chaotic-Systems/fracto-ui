...
const format_pixel_count = (image_size_px) => {
  ...
  if (pixel_count >= 1000000)
    return `${(pixel_count / 1000000).toFixed(2).replace(/\.00$/, "")}M px`;
...
export class TestAnimation extends Component {
  ...
  state = {
    animation_frame_rate_fps: AppSettings.get(
      KEY_TILES_TEST_ANIMATION_FRAME_RATE_FPS,
    ),
    animation_image_size_px: AppSettings.get(
      KEY_TILES_TEST_ANIMATION_IMAGE_SIZE_PX,
    ),
    animation_frame_count: AppSettings.get(
      KEY_TILES_TEST_ANIMATION_FRAME_COUNT,
    ),
    ...
    Promise.all(requests)
      .then((results) => {
        const records = results
          .flat()
          .filter(
            (record) => record && Number.isFinite(Number(record.magnitude)),
          )
          .sort(
            (left, right) => Number(right.magnitude) - Number(left.magnitude),
          );
        const candidates = records.slice(500, 1001);
        if (!candidates.length)
          throw new Error("No benchmark starting points available");
        const record =
          candidates[Math.floor(Math.random() * candidates.length)];
        const display_settings =
          typeof record.display_settings === "string"
            ? JSON.parse(record.display_settings)
        ...
        if (
          ...
          throw new Error(
            "Selected starting point has invalid display settings",
          );
        }
        if (!this.unmounted)
          this.setState({
            animation_frame_settings: {
              focal_point: {
                x: Number(focal_point.x),
                y: Number(focal_point.y),
              },
              scope,
            },
            animation_frame_index: 0,
  ...
  get_animation_scope = (frame_index) => {
    ...
    const factor =
      (final_scope / animation_frame_settings.scope) ** (1 / intervals);
  ...
  on_animation_operation = (operation) => {
    ...
    if (operation === TRANSPORT_BEGIN || operation === TRANSPORT_END)
      this.on_animation_stop();
  };
  ...
  on_animation_loading = (ctx, width_px, height_px) => {
    ...
    draw_loading_canvas(
      ...
      width_px,
      height_px,
      AppText.get(KEY_TILES_TEST_ANIMATION_LOADING),
    );
  };
  ...
  render() {
    ...
    const select_animation_setting =
      (key, state_key, custom_state_key) => (event) => {
    ...
    const frame_rate_value =
      animation_frame_rate_custom ||
      !FRAME_RATE_OPTIONS_FPS.includes(animation_frame_rate_fps)
        ? CUSTOM_OPTION
        : String(animation_frame_rate_fps);
    const image_size_value =
      animation_image_size_custom ||
      !IMAGE_SIZE_OPTIONS_PX.includes(animation_image_size_px)
        ? CUSTOM_OPTION
        : String(animation_image_size_px);
    const frame_count_value =
      animation_frame_count_custom ||
      !FRAME_COUNT_OPTIONS.includes(animation_frame_count)
        ? CUSTOM_OPTION
        : String(animation_frame_count);
    const render_stat_label = (label_key) => (
      <span>{AppText.get(label_key)}:</span>
    );

    const animation_image_column_width = Math.max(
      width_px / 2,
      animation_image_size_px + SCROLLBAR_WIDTH_PX,
    );
    const animation_stats = [
      {
        name: [render_stat_label, KEY_TILES_TEST_ANIMATION_FRAME_INDEX],
        value: animation_frame_index,
      },
      ...(animation_frame_settings
        ? [
            {
            ...
            {
              name: [render_stat_label, KEY_NAVIGATOR_SCOPE],
              value: [render_scalar, animation_frame_settings.scope],
            },
          ]
        : []),
    ];
        ...
        >
          <label style={SETTING_LABEL_STYLE}>
            ...
            <select
              ...
              onChange={select_animation_setting(
                KEY_TILES_TEST_ANIMATION_FRAME_RATE_FPS,
                "animation_frame_rate_fps",
                "animation_frame_rate_custom",
              )}
            ...
            >
              {FRAME_RATE_OPTIONS_FPS.map((value) => (
                <option key={value} value={value}>
                  {format_frame_rate(value)}
                </option>
              ))}
              <option value={CUSTOM_OPTION}>
                {AppText.get(KEY_TILES_TEST_ANIMATION_CUSTOM)}
              </option>
            </select>
            {frame_rate_value === CUSTOM_OPTION && (
              <input
                type={"number"}
                min={"1"}
                step={"1"}
                value={animation_frame_rate_fps}
                onChange={update_animation_setting(
                  KEY_TILES_TEST_ANIMATION_FRAME_RATE_FPS,
                  "animation_frame_rate_fps",
                )}
          ...
          <label style={SETTING_LABEL_STYLE}>
            ...
            <select
              ...
              style={{ marginLeft: "0.35rem" }}
            >
              {IMAGE_SIZE_OPTIONS_PX.map((value) => (
                <option key={value} value={value}>
                  {value} ({format_pixel_count(value)})
                </option>
              ))}
              <option value={CUSTOM_OPTION}>
                {AppText.get(KEY_TILES_TEST_ANIMATION_CUSTOM)}
            ...
            {image_size_value === CUSTOM_OPTION && (
              <input
                type={"number"}
                min={"1"}
                step={"1"}
                value={animation_image_size_px}
                onChange={update_animation_setting(
                  KEY_TILES_TEST_ANIMATION_IMAGE_SIZE_PX,
                  "animation_image_size_px",
                )}
          ...
          <label style={SETTING_LABEL_STYLE}>
            <span style={CELL_LABEL_STYLE}>
              {AppText.get(KEY_TILES_TEST_ANIMATION_FRAME_COUNTER)}
            </span>
            <select
            ...
            {frame_count_value === CUSTOM_OPTION && (
              <input
                type={"number"}
                min={"1"}
                step={"1"}
                value={animation_frame_count}
                onChange={update_animation_setting(
                  KEY_TILES_TEST_ANIMATION_FRAME_COUNT,
                  "animation_frame_count",
                )}
