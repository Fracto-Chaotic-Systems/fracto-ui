...
import { GENERATOR_CODE_REDO } from "./GeneratorControl.jsx";
...
export class GeneratorHistory extends Component {
  ...
  render_boolean = (value) => {
    ...
    return value ? (
      <styles.CenteredCell style={checkmark_style}>
        {checkmark_icon}
      </styles.CenteredCell>
    ) : (
      []
  ...
  render_summary_line_1 = () => {
    ...
    const text =
      subset_records.length > 0
        ? `${run_count} results this run (${rate_str}): ${stats_str}`
        : "";
    return <SummaryRow style={extra_style}>{text}</SummaryRow>;
  };
  ...
  render_summary_line_2 = () => {
    ...
    if (subset_records.length) {
      ...
      if (run_count && run_start) {
        ...
        const time_to_complete =
          ((timer_now - run_start) * (tile_count - run_count)) /
          (run_count + 1);
        const now = Date.now();
    ...
    return <SummaryRow style={extra_style}>{time_stats}</SummaryRow>;
  };
  ...
  render() {
    ...
    const table_data = subset_records.map((record, i) => {
      return {
    ...
    return (
      <styles.ContentWrapper style={wrapper_style}>
        {summary_line_1}
        {summary_line_2}
        <styles.HalfRemDown />
        <CoolTable columns={TABLE_COLUMNS} data={table_data} options={[]} />
