...
import {
  KEY_TILES_STATUS,
  KEY_TILES_STATUS_EFFECTIVENESS,
...
export class TilesStatus extends Component {
  ...
  get_ratio_data = () => {
    ...
    return [
      ...
      {
        metric: "local hit ratio",
        value: ratio(memory_hits + disk_hits, requests),
      },
      {
        metric: "download / miss ratio",
        value: ratio(Number(stats?.downloads || 0), requests),
      },
      {
        metric: "failure ratio",
        value: ratio(Number(stats?.failures || 0), requests),
      },
    ];
  };
  ...
  render() {
    ...
    const chart_width = Math.max(
      250,
      Math.floor(rendered_width / 2) - 4 * MARGIN_PX,
    );
    ...
    const available_height =
      rendered_height - top + TITLE_BAR_HEIGHT_PX - 2 * MARGIN_PX;
    ...
    const chart_height = Math.max(
      ...
      Math.min(
        420,
        Math.floor(
          (available_height - CHART_CONTENT_RESERVE_PX) /
            (charts_stacked ? 2 : 1),
        ),
      ),
    );
    return [
      <styles.SectionTitle key={"tiles-status-title"}>
      ...
      <styles.CenteredBlock key={"tiles-status-content"} ref={container_ref}>
        ...
        {stats && (
          <>
            ...
            <styles.TableWrapper>
              <CoolTable columns={RATIO_COLUMNS} data={this.get_ratio_data()} />
            </styles.TableWrapper>
          </>
        )}
        <div
        ...
        >
          ...
          >
            <Line
              data={this.get_history_chart_data()}
        ...
        {history.length > 0 && (
          <div style={{ color: "#777777", fontSize: "0.8rem" }}>
            {history.length} history sample{history.length === 1 ? "" : "s"}{" "}
            collected while this page was active
          </div>
        )}
