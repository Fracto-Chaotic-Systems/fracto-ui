...
export class InputForm extends Component {
  ...
  init_values = () => {
    ...
    form_entries.forEach((entry) => {
      const setting_definition =
        AppSettings.setting_definitions[entry.settings_key];
  ...
  render() {
    ...
    const all_entries = form_entries.map((entry, i) => {
      ...
      const label_style = entry.label_width_px
        ? { width: `${entry.label_width_px}px` }
        : {};
      return (
        <styles.CenteredBlock key={`input-form_entries_${i}`}>
    ...
    const elements = [title_block, all_entries, button_block];
    return <styles.FormWrapper>{elements}</styles.FormWrapper>;
  }
}
