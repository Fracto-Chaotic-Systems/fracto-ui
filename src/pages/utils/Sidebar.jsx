...
export class Sidebar extends Component {
  ...
  render() {
    ...
    const sidebar = sidebar_list.map((item, i) => {
      if (item.section_code === SIDEBAR_BREAKER) {
        if (item.section_text_key) {
          ...
          return (
            <styles.SidebarBreaker key={`breaker-${i}`}>
      ...
      const item_style =
        item.section_code === section_code
          ? {
              background:
                "linear-gradient(15deg, #557799 0%, #7799bb 50%, #bbddff 90%)",
            }
          : {};
      const title_style =
        item.section_code === section_code
      ...
      return (
        ...
        >
          <styles.SidebarLink style={title_style}>
            {item.title}
          </styles.SidebarLink>
        </styles.SidebarItem>
      );
    ...
    return (
      <styles.PaneWrapper style={pane_style}>{sidebar}</styles.PaneWrapper>
    );
  }
}
