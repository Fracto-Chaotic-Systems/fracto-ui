...
const render_meta_data = (obj_meta) => {
  ...
  const button_content = content.id < 0 ? "create" : `update id=${content.id}`;
  const blue_button = (
    <CoolButton
  ...
  const all_elements = [
    ...
    return <styles.MetaElement>{element}</styles.MetaElement>;
  });
  return (
    <CoolStyles.Block style={{ lineHeight: "1.75rem" }}>
      {all_elements}
    </CoolStyles.Block>
  );
};
