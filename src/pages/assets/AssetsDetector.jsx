...
import { detect_now, highlight_existing } from "./detector/DetectorUtils.js";
...
export class AssetsDetector extends Component {
  ...
  componentDidMount() {
    this.load_minibrots();
    this.setState({
      frame_settings: AppSettings.get(KEY_ASSETS_DETECTOR_FRAME_SETTINGS),
  ...
  detect_now = () => {
    ...
    if (new_bailiwick) {
      MinibrotBackend.save_bailiwick(new_bailiwick, 0, (response) => {
  ...
  render_nodal_check = () => {
    ...
    const checkmark = is_node ? (
      <CoolStyles.InlineBlock style={checkmark_style}>
        {checkmark_icon}
    ...
    return (
      <styles.NormalLink onClick={this.set_nodal}>
        {checkmark} nodal
      </styles.NormalLink>
    );
  };
  ...
  render_right_block = (top) => {
    ...
    const detect_button = (
      <styles.BlueButton style={detect_button_style} onClick={this.detect_now}>
        detect now
      </styles.BlueButton>
    );
    const enhance_button = (
      <styles.BlueButton onClick={this.enhance}>enhance</styles.BlueButton>
    );
