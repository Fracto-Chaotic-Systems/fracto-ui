...
export const find_minima = (frame_settings) => {
  ...
  return Object.values(pattern_minima)
...
export const highlight_existing = (minibrot_list, frame_settings) => {
  ...
  const minibrots_in_field = minibrot_list.filter((minibrot) => {
  ...
  minibrots_in_field
    ...
    .forEach((minibrot, i) => {
      const core_point = JSON.parse(minibrot.core_point);
      const x =
        ((core_point.x - leftmost) * frame_settings.width_px) /
        frame_settings.scope;
      const y =
        ((topmost - core_point.y) * frame_settings.width_px) /
        frame_settings.scope;
      let width_px = 0;
      let color = "#FFFFFF00";
      let line_width = 0;
      ...
      ctx.strokeRect(x - width_px / 2, y - width_px / 2, width_px, width_px);
