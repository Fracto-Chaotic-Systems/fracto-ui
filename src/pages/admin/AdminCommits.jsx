...
import {CELL_ALIGN_CENTER, CELL_ALIGN_LEFT, CELL_ALIGN_RIGHT, CELL_LABEL_STYLE, CELL_TYPE_CALLBACK, CELL_TYPE_NUMBER, CELL_TYPE_TEXT} from "../../utils/ui/styles/CoolTableStyles.jsx";
import {
...
const COMMIT_FIELDS = [
   ['repository', KEY_ADMIN_COMMIT_REPOSITORY, CELL_TYPE_CALLBACK, CELL_ALIGN_CENTER],
   ...
   ['change_summary', KEY_ADMIN_COMMIT_CHANGES, CELL_TYPE_CALLBACK, CELL_ALIGN_LEFT],
]
...
const render_repository_link = repository => {
   ...
   return <a
      ...
      onMouseLeave={event => { event.currentTarget.style.color = 'black' }}
      style={{fontWeight: 'bold', letterSpacing: '1px', color: 'black', cursor: 'pointer', textTransform: 'uppercase'}}>
      {repository_label(repository)}
   </a>
}
const change_summary_text = commit => `${commit.files_changed} ${text_label(KEY_ADMIN_COMMIT_FILES)}: ` +
...
const render_change_summary = commit => {
   const number_style = {fontFamily: 'monospace', fontWeight: 'bold', color: 'black'}
   const text_style = {fontFamily: 'sans-serif', fontStyle: 'italic', color: '#666666'}
   return <>
      <span style={number_style}>{commit.files_changed}</span> <span style={text_style}>{text_label(KEY_ADMIN_COMMIT_FILES)}:</span>{' '}
      <span style={number_style}>{commit.insertions}</span> <span style={text_style}>{text_label(KEY_ADMIN_COMMIT_INSERTED)},</span>{' '}
      <span style={number_style}>{commit.deletions}</span> <span style={text_style}>{text_label(KEY_ADMIN_COMMIT_DELETED)},</span>{' '}
      <span style={number_style}>{commit.files_created}</span> <span style={text_style}>{text_label(KEY_ADMIN_COMMIT_CREATED)},</span>{' '}
      <span style={number_style}>{commit.files_removed}</span> <span style={text_style}>{text_label(KEY_ADMIN_COMMIT_REMOVED)}</span>
   </>
}
const render_commit_hash = ({hash, repository}) => {
   ...
   return <>
      <a
         ...
         onMouseEnter={event => { event.currentTarget.style.color = 'blue' }}
         onMouseLeave={event => { event.currentTarget.style.color = 'black' }}
         style={{fontFamily: 'monospace', fontWeight: 'bold', letterSpacing: '1px', color: 'black', cursor: 'pointer'}}>
         {hash_label(hash)}
      </a>
      <styles.InlineHover
         ...
         {React.cloneElement(copy, {width: '1rem', height: '1rem', fill: 'currentColor'})}
      </styles.InlineHover>
   </>
}
...
const commit_columns = (commits, available_width) => {
   const intrinsic = COMMIT_FIELDS.map(([id, label_key]) => {
      const widest = Math.max(
         ...
         ...commits.map(commit => {
            if (id === 'repository') return repository_label(commit[id]?.[1] || commit[id]).length
            if (id === 'hash') return hash_label(commit[id]?.hash || commit[id]).length
            if (id === 'date') return 'moments ago'.length
            if (id === 'change_summary') return change_summary_text(commit[id]?.[1] || commit[id]).length
            return text_value(commit[id]).length
         }),
      )
      const character_width = id === 'message'
         ? MESSAGE_CHARACTER_WIDTH_PX
         : MONOSPACE_CHARACTER_WIDTH_PX
      return Math.max(48, widest * character_width + COLUMN_HORIZONTAL_PADDING_PX)
   })
...
export class AdminCommits extends Component {
   ...
   set_all_repositories = visible => {
      const repository_visibility = Object.fromEntries(REPOSITORY_NAMES.map(name => [name, visible]))
      this.setState({repository_visibility}, () => {
         AppSettings.on_settings_changed({
   ...
   render() {
      ...
      const any_repository_visible = REPOSITORY_NAMES.some(repository => repository_visibility[repository] !== false)
      const any_repository_hidden = REPOSITORY_NAMES.some(repository => repository_visibility[repository] === false)
      const commit_rows = visible_commits.map(commit => ({
         ...commit,
         repository: [render_repository_link, commit.repository],
         hash: [render_commit_hash, {hash: commit.hash, repository: commit.repository}],
         date: [render_commit_date, commit.date],
         change_summary: [render_change_summary, commit],
      }))
      return <>
         <styles.SectionTitle
            style={{position: 'sticky', top: 0, zIndex: 3, backgroundColor: '#fcfcfc'}}
            key={'admin-commits-title'}>
            {AppText.get(KEY_ADMIN_COMMITS_TITLE)}
         </styles.SectionTitle>
         <CoolStyles.Block ref={table_ref} style={{width: '100%', height: 'calc(100% - 75px)', overflow: 'hidden'}}>
            <CoolStyles.Block style={{height: `${TABLE_HEADER_SPACE_PX}px`, display: 'flex', alignItems: 'center', gap: '1rem', padding: '0 1rem', position: 'sticky', top: 0, zIndex: 2, backgroundColor: '#fcfcfc'}}>
               {REPOSITORY_NAMES.map(repository => <label
                  ...
                  /><span style={{marginLeft: '0.35rem', ...CELL_LABEL_STYLE}}>{repository_label(repository)}</span>
               </label>)}
               <a
                  href="#"
                  onClick={event => {
                  ...
                  style={{...CELL_LABEL_STYLE, color: any_repository_hidden ? 'blue' : '#999999', cursor: any_repository_hidden ? 'pointer' : 'default'}}>
               ...
               <a
                  ...
                  style={{...CELL_LABEL_STYLE, color: any_repository_visible ? 'blue' : '#999999', cursor: any_repository_visible ? 'pointer' : 'default'}}>
            ...
            {!!commits.length && <CoolTable
               ...
               data={commit_rows}
               table_style={{width: '100%', height: `calc(100% - ${TABLE_HEADER_SPACE_PX}px)`, backgroundColor: 'white', color: 'black'}}
            />}
         </CoolStyles.Block>
      </>
   }
