...
import {
  KEY_DATA_CONTENT_QUERIES,
  KEY_DATA_QUERIES_LOAD_ERROR,
  KEY_DATA_QUERIES_LOADING,
  ...
  return fields.map((field) => {
    ...
    const width_px = Math.ceil(
      Math.max(
        ...
        Math.max(
          field.length,
          ...records.map((record) => cell_text(record[field]).length),
        ) *
...
export class AdminQueries extends Component {
  ...
  componentDidMount() {
    ...
    const tab_index =
      Number.isInteger(saved_tab) &&
      saved_tab >= 0 &&
      saved_tab < TABLE_TABS.length
        ? saved_tab
        : 0;
  ...
  componentWillUnmount() {
    if (this.state.dimensions_interval)
      clearInterval(this.state.dimensions_interval);
    this.unmounted = true;
  }
  ...
  load_table = async (tab_index) => {
    ...
    this.setState((previous) => ({
      loading: previous.loading.map((value, index) =>
        index === tab_index ? true : value,
      ),
      errors: previous.errors.map((value, index) =>
        index === tab_index ? null : value,
      ),
    }));
    try {
      ...
      this.setState((previous) => ({
        records: previous.records.map((value, index) =>
          index === tab_index ? payload.result || [] : value,
        ),
        loading: previous.loading.map((value, index) =>
          index === tab_index ? false : value,
        ),
      }));
    } catch (error) {
      if (!this.unmounted)
        this.setState((previous) => ({
          ...
          errors: previous.errors.map((value, index) =>
            index === tab_index ? error.message : value,
          ),
        }));
    }
  };
  ...
  render() {
    ...
    const selected_content = (
      <CoolStyles.Block
        style={{ backgroundColor: "white", minHeight: "12rem" }}
      >
        <CoolStyles.Block style={{ height: `${TABLE_HEADER_SPACE_PX}px` }} />
        {loading[tab_index] && (
          <CoolStyles.Block>
            {AppText.get(KEY_DATA_QUERIES_LOADING)}
          </CoolStyles.Block>
        )}
        {table_error && (
          <CoolStyles.Block style={{ color: "#b22222" }}>
            {AppText.get(KEY_DATA_QUERIES_LOAD_ERROR)} {table_error}
          </CoolStyles.Block>
        )}
        {!loading[tab_index] && !table_error && (
          <CoolTable
    ...
    const field_height = rendered_height
      ? Math.max(0, rendered_height - field_top)
      : undefined;
    return (
      <styles.PaneWrapper>
        <styles.SectionTitle>
