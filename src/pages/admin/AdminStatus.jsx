...
const status_cell = status => {
   ...
   return <span style={{color: STATUS_COLORS[value] || STATUS_COLORS.unavailable, fontWeight: 'bold'}}>{value}</span>
}
const TABLE_COLUMNS = [
   {id: 'service', label: 'service:', type: CELL_TYPE_TEXT, width_px: 230, align: CELL_ALIGN_LEFT,
...
export class AdminStatus extends Component {
   state = {health: null, readiness: null, error: null, updated_at: null, interval: null, refreshing: false}
   ...
   render() {
      ...
      return [
         <styles.SectionTitle key={'admin-status-title'}>{AppText.get(KEY_ADMIN_STATUS)}</styles.SectionTitle>,
         <styles.CenteredBlock key={'admin-status-content'}>
            ...
            {error && <>
               ...
               <styles.ButtonBlock>
                  <styles.HoverBlueButton onClick={this.refresh} disabled={refreshing}>
                     {refreshing ? 'checking...' : AppText.get(KEY_ADMIN_STATUS_REFRESH)}
                  </styles.HoverBlueButton>
               </styles.ButtonBlock>
            </>}
            <styles.TableWrapper><CoolTable columns={TABLE_COLUMNS} data={rows}/></styles.TableWrapper>
