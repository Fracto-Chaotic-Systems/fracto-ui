...
const AspectPatternSpan = styled(CoolStyles.InlineBlock)`
...
export const identify_cores = (frame_settings) => {
  ...
  for (
    let canvas_x = 0;
    canvas_x < frame_settings.canvas_buffer.length;
    canvas_x++
  ) {
    for (
      let canvas_y = 0;
      canvas_y < frame_settings.canvas_buffer[canvas_x].length;
      canvas_y++
    ) {
...
export const render_magnitude = (magnitude, is_pico = false) => {
  ...
  const mu = is_pico ? <i>{"p"}</i> : <i>{"\u03BC"}</i>;
  return (
    <styles.NumericValue title={magnitude} style={{ verticalAlign: "middle" }}>
      {" "}
      {rounded}
...
export const render_pattern_block = (pattern, font_size_px = 18) => {
  ...
  return (
    <styles.PatternBlock style={block_style}>{pattern}</styles.PatternBlock>
  );
};
...
export const render_aspect_badge = (aspect, pattern, font_size_px = 18) => {
  ...
  const block_style = {
    textShadow: "3px 3px 6px rgba(0,0,0,0.75)",
    backgroundColor: pattern_color,
    borderRadius: `${font_size_px / 2}px`,
    border: `1px solid black`,
    width: `${font_size_px * 1.41}px`,
    height: `${font_size_px * 1.41}px`,
    transform: `rotate(45deg)`,
    opacity: 0.65,
  };
  const pattern_style = {
    left: `-${-1 + font_size_px * 1.41}px`,
    fontSize: `${font_size_px}px`,
    width: `${font_size_px}px`,
    zIndex: 1000,
  };
  return [
    <CoolStyles.InlineBlock style={block_style}></CoolStyles.InlineBlock>,
    <AspectPatternSpan style={pattern_style}>{aspect}</AspectPatternSpan>,
  ];
};
