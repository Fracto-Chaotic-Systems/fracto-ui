...
export class PointsMainPanel extends Component {
   ...
   test_theory = (big_points, P_coords) => {
      const point_list = big_points.point_list.slice(-(big_points.cardinality + 1))
      console.log(`Test points ${point_list.length}`, point_list)
      const all_points = point_list.map(data => {
         return new Complex(
            parseFloat(data.point.re),
            parseFloat(data.point.im),
         )
      })
      ...
      console.log('test_theory, sum, sum_squared_add_P, sum_squared_minus_sum_add_P',
         sum.toString(),
   ...
   on_chart_update = (key, value) => {
      ...
      DataBackend.get_orbitals(value.focal_point, 50000, all_results => {
         ...
         const newton_chart_data = this.format_point_data(newton_derived, newton_derived.cardinality)
         ...
         console.log('retro_derived, retro_chart_data', retro_derived, retro_chart_data)
         console.log('newton_derived, newton_chart_data', newton_derived, newton_chart_data)
         this.setState({
            pro_derived,
            retro_derived,
            newton_derived,
   ...
   render() {
      const {
         in_fetch,
         pro_derived, retro_derived, newton_derived,
         pro_chart_data, retro_chart_data, newton_chart_data,
      } = this.state
