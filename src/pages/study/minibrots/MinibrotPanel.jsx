...
import MinibrotList, { TABLE_WIDTH_PX } from "./MinibrotList.jsx";
...
export class MinibrotPanel extends Component {
  ...
  componentDidMount() {
    this.setState({
      rendered_splitter_pos: AppSettings.get(
  ...
  change_splitter_pos = (pos) => {
    ...
    if (container_ref.current) {
      ...
      if (
        pos >
        container_bounds.left + TABLE_WIDTH_PX + container_bounds.height / 2
      ) {
        return;
      }
    }
  ...
  render() {
    ...
    return (
      <styles.TightCenteredBlock ref={container_ref}>
        <styles.ScrollingBlock style={table_style} key={"input-form"}>
          {minibrot_table}
          {left_panel}
          <CoolSplitter
