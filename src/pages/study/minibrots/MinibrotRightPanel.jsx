...
export class MinibrotRightPanel extends Component {
  ...
  title_bar = () => {
    ...
    const pattern_block = render_pattern_block(selected_minibrot.pattern, 42);
    ...
    const name_block = (
      <styles.NumericValue style={minibrot_name_style}>
  ...
  legend_bar = () => {
    ...
    const legend = <FractoLegend height_px={150} focal_point={core_point} />;
    ...
    const data_table = (
      <styles.FixedInlineBlock style={{ marginLeft: "0.5rem" }}>
        <CoolTable
  ...
  render() {
    ...
    const splitter_pos = AppSettings.get(
      KEY_STUDY_MINIBROTS_RENDER_SPLITTER_POS,
    );
    const main_splitter_pos = AppSettings.get(KEY_STUDY_SPLITTER_POS_PX);
    const panel_width_px =
      container_bounds.width - splitter_pos + main_splitter_pos;
    ...
    const blocks = [title_bar, legend_bar].map((block, i) => {
      return (
        <CoolStyles.Block style={block_style} key={`block-${i}`}>
          {block}
        </CoolStyles.Block>
      );
    });
    return (
      <CoolStyles.Block style={{ height: `${container_bounds.height}px` }}>
        {blocks}
      </CoolStyles.Block>
    );
  }
