...
export class StudyMeridians extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      frame_settings: AppSettings.get(KEY_STUDY_MERIDIANS_FRAME_SETTINGS),
  ...
  render_content = () => {
    ...
    for (let cardinality = 3; cardinality <= MAX_CARDINALITY; cardinality++) {
      const aspects = farey_sequence.filter(
        (element) => element.den === cardinality,
      );
      const cardinality_block = (
        <MeridianBlock cardinality={cardinality} aspects={aspects} />
      );
      all_blocks.push(cardinality_block);
    }
    return <BlocksBackground>{all_blocks}</BlocksBackground>;
  };
  ...
  render() {
    ...
    return [
      ...
      <styles.TightCenteredBlock ref={container_ref} key={"meridians-layout"}>
