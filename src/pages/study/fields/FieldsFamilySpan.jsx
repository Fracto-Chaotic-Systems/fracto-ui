...
export class FieldsFamilySpan extends Component {
  ...
  componentDidMount() {
    ...
    const interval = setInterval(() => {
      const settings_changed = PageSettings.test_update_settings(
        [KEY_UPDATE_INDEX, KEY_SCOPE, KEY_FOCAL_POINT],
        page_settings,
        stored_values,
      );
      if (settings_changed) {
  ...
  fill_pattern_bins = () => {
    const { canvas_ref, wrapper_width_px, wrapper_height_px, height_scalar } =
      this.state;
    const { page_settings } = this.props;
    ...
    Object.keys(orbital_bins).forEach((key) => {
      ...
      const family = FractoUtil.fracto_pattern_family(
        orbital_bins[key].orbital,
      );
    ...
    Object.keys(families).forEach((key) => {
      ...
      const leftmost =
        family.base === 1
          ...
          : Math.round(
              wrapper_width_px *
                (Math.log2(family.base) - Math.floor(Math.log2(family.base))),
            );
      let base_y = wrapper_height_px;
      ctx.fillStyle = FractoUtil.fracto_pattern_color(family.base);
      family.members
        .sort((a, b) => a.orbital - b.orbital)
        .forEach((member) => {
          const height_px =
            (height_scalar * wrapper_height_px * Math.log(member.bin_count)) /
            log_highest_count;
          base_y -= height_px;
