...
export class DataBackend {
   ...
   */
  static radian_data = (theta_num = 5, theta_den = 11, precision = 24) =>
    request_json(
  ...
  static get_minibrots = (params, cb) => {
    setTimeout(async () => {
      const all_params = Object.keys(params)
        .map((key) => {
  ...
  static lore_storage = async (content) => {
    const { content_data, content_meta, id, title, category, category_key } =
      content;
