...
export class StudyMagnitudes extends Component {
  ...
  get_vector_data = async () => {
    const vector_data = await OrbitalMagnitudes.read_vector_data(1, 3, 24);
  ...
  get_filtered_data = () => {
    ...
    return vector_data.file_contents
      .filter((value) => {
        return parseFloat(value.r) > 0.999 && parseFloat(value.r) < 1.0;
      })
  ...
  render_magnitudes_chart = (rendered_width, rendered_height) => {
    ...
    const magnitudes = OrbitalMagnitudes.magnitudes_chart(
      filtered_data,
      this.on_hover,
    );
    return (
      <styles.InlineContentWrapper style={background_style}>
        {magnitudes}
  ...
  render() {
    ...
    const splitter_width = AppSettings.get(KEY_STUDY_SPLITTER_POS_PX);
    const magnitudes_chart = this.render_magnitudes_chart(
      rendered_width,
      rendered_height,
    );
    ...
    return [
      ...
      <styles.CenteredBlock style={contents_style} key={"input-form"}>
        ...
        >
          <FareySequenceList height_px={rendered_height * 0.4} />
        </styles.InlineContentWrapper>
        <styles.OneRemSpacer />
        <styles.InlineContentWrapper
          style={charts_style}
