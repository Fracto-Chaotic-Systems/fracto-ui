...
export class AssetsGeneratorNot extends Component {
  ...
  componentDidMount() {
    const frame_settings = AppSettings.get(KEY_ASSETS_GENERATOR_FRAME_SETTINGS);
    this.setState({
  ...
  render_image = async () => {
    ...
    const image_outcome = await AssetsBackend.render_image(
      frame_settings,
      resolution,
      selected_level,
    );
  ...
  render_button_block = () => {
    const { image_outcome, insert_outcome, coverage_data, resolution } =
      this.state;
    if (!coverage_data) {
      return [];
    }
    ...
    const render_now_button = (
      <CoolStyles.Block onClick={this.render_image} key={"resolution-select"}>
        <styles.BlueButton key={"blue-button"}>
          {AppText.get(KEY_IMAGE_ASSETS_RENDER_NOW)}
        </styles.BlueButton>
      </CoolStyles.Block>
    );
    const add_to_gallery_button =
      image_outcome && !insert_outcome ? (
        <CoolStyles.Block onClick={this.add_to_gallery} key={"add-to-gallery"}>
          <styles.BlueButton key={"blue-button"}>
  ...
  render() {
    ...
    const image = image_outcome?.public_url ? (
      <styles.FixedInlineBlock key={"rendered-image"} style={image_style}>
        <img
          onClick={(e) =>
            window.open(
              image_outcome.public_url,
              "_blank",
              "noopener,noreferrer",
            )
          }
    ...
    return [
      <styles.SectionTitle key={"assets-overview-title"}>
